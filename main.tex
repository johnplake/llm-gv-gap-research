...
\newcommand\jd[1]{\todo[color=blue!40]{{\bf JD}: #1}}

\newcommand{\juandiego}[1]{\textcolor{blue}{\textbf{JD:} #1}}

\newcommand{\agent}[1]{\textcolor{brown}{\textbf{AI:} #1}}

\title{Improving LM Generators via Validator-to-Generator Training}
...
\begin{itemize}
    \item We identify methodological issues in prior work on ranking alignment, particularly the comparison of pairs across different prompts. (OK this is maybe beating up too much on RankAlign?)
    \item We propose a same-prompt training constraint that ensures valid preference comparisons.
    \item We demonstrate improvements on lexical semantic, instruction-following (COLLIE), and question-answering tasks.
\end{itemize}
...
Prior work has documented the gap between generation and validation capabilities in LLMs~\citep{west2022symbolic}. Specifically...\citep{li-benchmarking2024,rodriguez2025rankalign}
...
% Here, trying to find new tasks:
% Some candidates:


% 1. Laban et al., Near-negative distinction: giving a second life to human evaluation datasets

% 2. Jennifer Hu's datasets from "Auxiliary task demands mask the capabilities of smaller language models"

% 3. Personality datasets , i.e., the ones used in Eric Bigelow et al.. Belief Dynamics Reveal the Dual Nature of In-Context Learning and Activation Steering (https://arxiv.org/abs/2511.00617 )
