"""Download QuAC (allenai/quac) from Hugging Face and materialize to disk.

NOTE: QuAC uses a dataset loading script on HF; newer `datasets` versions may reject
script-based datasets. We pin to a known-working version when running via `uv`.
...
def main() -> None:
    ...
    ds_dict = load_dataset(hf_id)  # script-based dataset
    ...
    _write_json(out_dir / "quac" / "manifest.json", manifest)

    for split, ds in ds_dict.items():
        split_dir = out_dir / "quac" / split
    ...
    print(f"✅ Wrote quac to {out_dir/'quac'}")
